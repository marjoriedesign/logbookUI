...
- `CorrectionsTable` extrait en vrai composant exporté (`src/components/CorrectionsTable.tsx`) : n'existait auparavant que comme code de démo inline dans sa story, pas consommable ailleurs. API contrôlée par props (`rows` + callbacks `onToggleSubmitted`/`onCorrect`/`onAnalyze`/`onOpenStudent`/`onOpenActions`) ; le tri par nom reste un état interne purement présentationnel. Réexporté dans le barrel public.
