...
- Doc "Tokens & repères" ajoutée sur les 21 pages Components/Logbook (`PageLayout` accepte `tokens`/`notes`) : quels tokens chaque composant consomme + décisions/exceptions importantes (corrections AA, overrides de comportement natif MUI, valeurs figées justifiées, incohérences connues comme `primary.dark` vert sur IconButton, tokens définis mais non consommés comme `typography.chip.label`) — pour que Marjorie configure Tokens Studio/Figma composant par composant et qu'un dev retrouve le contexte sans relire l'historique de commits.
